...
\title{Haskell 101}
\author{ningningxie@, ibobyr@, nicuveo@}
%\institue{Google}
\date{\small\today}
...
\begin{frame}
  ...
  \begin{center}
  \begin{itemize}
    \item Ph.D. in programming languages
    \item Haskell user/researcher
    \item Modest GHC contributor
  \end{itemize}
  \end{center}
\end{frame}
...
\bi{\ic{length}} &\ic{.}& \bi{\ic{show}} &\ic{::}& \type{\ic{Stuff}} &       &             &\ic{->}& \ic{Int} \\
  \end{tabular}
\end{onlyenv}
\only<13->{\ic{(} \bi{\ic{length}} \ic{(} \bi{\ic{show}} \ic{(} \bi{\ic{filter}} \ic{ p (}  \bi{\ic{map}} \ic{ f xs))}\\}
\only<14>{\bi{\ic{length}} \ic{.} \bi{\ic{show}} \ic{.} \bi{\ic{filter}} \ic{ p .}  \bi{\ic{map}} \ic{ f \$ xs} }
\end{center}
\end{minipage}
\end{frame}
